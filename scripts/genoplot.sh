...
Rscript "$SCRIPT_DIR/scripts/depth_analysis.R" "${ARGS[@]}"
...
Rscript "$SCRIPT_DIR/scripts/vcf_amplicon_geno_clairs-to.R" "$ANALYSIS_DIR/ClairS-TO/" "$MOD_STRP${MAPQ_MOD}_$CLAIR_MODEL" "$TXFILE" "$OUT_DIR" "snv" "${SAMPLES[@]}"
Rscript "$SCRIPT_DIR/scripts/vcf_amplicon_geno_clairs-to.R" "$ANALYSIS_DIR/ClairS-TO/" "$MOD_STRP${MAPQ_MOD}_$CLAIR_MODEL" "$TXFILE" "$OUT_DIR" "indel" "${SAMPLES[@]}"
...
Rscript "$SCRIPT_DIR/scripts/genotyping.R" "$ANALYSIS_DIR/ClairS-TO/" "${MOD_STRP}${MAPQ_MOD}_${CLAIR_MODEL}_snv_vcf_collection.tsv" "$OUT_DIR"
